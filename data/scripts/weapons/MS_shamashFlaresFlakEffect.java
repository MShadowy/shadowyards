...
public class MS_shamashFlaresFlakEffect implements EveryFrameWeaponEffectPlugin, OnFireEffectPlugin {
    
    private static final float FLARE_DAMAGE = 100f;
    private static final float FLARE_FLAK_SIZE = 65f;
    ...
    private static final float FLARE_FLASH_DUR = 0.2f;
    private static final float FLARE_FUSE_RANGE = 30f;
    private static final float FLARE_VISUAL_SIZE = 90f;
    private static final float FLARE_CORE_SIZE = 30f;
    ...
    public static void burstFlakExplode(DamagingProjectileAPI projectile, Vector2f point, CombatEngineAPI engine)
    {
        ...
        engine.addHitParticle(point, ZERO, FLARE_CORE_SIZE, 1f, FLARE_FLASH_DUR, Color.WHITE);
        engine.addHitParticle(point, ZERO, FLARE_VISUAL_SIZE, 0.4f, 0.6f, FX_COLOR1);
        Vector2f vel = new Vector2f();
        for (int i = 0; i < 30; i++)
        {
            vel.set(((float) Math.random() * 1.25f + 0.25f) * FLARE_VISUAL_SIZE, 0f);
        ...
        light.setSize(FLARE_VISUAL_SIZE * 1.1f);
        ...
        List<ShipAPI> ships = CombatUtils.getShipsWithinRange(point, FLARE_FLAK_SIZE);
        List<CombatEntityAPI> targets = CombatUtils.getAsteroidsWithinRange(point, FLARE_FLAK_SIZE);
        targets.addAll(CombatUtils.getMissilesWithinRange(point, FLARE_FLAK_SIZE));
        ...
        for (CombatEntityAPI tgt : targets)
        {
            ...
            float reduction = 1f;
            if (distance > FLARE_FUSE_RANGE)
            {
                reduction = (FLARE_FLAK_SIZE - distance) / (FLARE_FLAK_SIZE - FLARE_FUSE_RANGE);
            }
            ...
            engine.applyDamage(tgt, damagePoint, FLARE_DAMAGE * reduction, DamageType.FRAGMENTATION, 0f, false, false, projectile.getSource());
        }
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        while (iter.hasNext()) {
            ...
            List<CombatEntityAPI> asteroids = CombatUtils.getAsteroidsWithinRange(loc, FLARE_FLAK_SIZE);
            toCheck.addAll(CombatUtils.getShipsWithinRange(loc, FLARE_FLAK_SIZE));
            toCheck.addAll(CombatUtils.getMissilesWithinRange(loc, FLARE_FLAK_SIZE));
            ...
            for (CombatEntityAPI entity : toCheck) {
                ...
                if (entity.getShield() != null)
                {
                                ...
                                && shield.isWithinArc(ahead))   // Yes, we are
                    {
                        ...
                        {
                            DO_NOT_EXPLODE.add(proj);
                            MS_shamashFlaresFlakEffect.burstFlakExplode(proj, loc, engine);
                        }
                    }
                }
