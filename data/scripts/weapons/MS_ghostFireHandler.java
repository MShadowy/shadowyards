...
public class MS_ghostFireHandler implements EveryFrameWeaponEffectPlugin, OnFireEffectPlugin {
    ...
    private void reacquireTarget(Vector2f point, DamagingProjectileAPI proj) {
	CombatEntityAPI newTarget = null;
	Vector2f centerOfDetection = point;
    ...
    private void applySwarmOffset(CombatEntityAPI targ) {
	...
	while (i > 0 && target != null) {
		...
		Vector2f potPoint = MathUtils.getRandomPointInCircle(targ.getLocation(), targ.getCollisionRadius());
		if (CollisionUtils.isPointWithinBounds(potPoint, targ)) {
		//If the point is valid, convert it to an offset and store it
			potPoint.x -= targ.getLocation().x;
			potPoint.y -= targ.getLocation().y;
			potPoint = VectorUtils.rotate(potPoint, -targ.getFacing());
